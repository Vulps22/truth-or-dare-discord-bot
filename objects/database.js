	...
	async connect(retryAttempt = 0) {
		try {
			this.connection = mysql.createConnection({
				host: process.env['ALPHA'] || process.env['BETA'] ? 'localhost' : process.env['DATABASE_HOST'],
				//host: process.env['DATABASE_HOST'],
				port: process.env['ALPHA'] || process.env['BETA'] ? process.env['DATABASE_LOCAL_PORT'] : process.env['DATABASE_DOCKER_PORT'],
